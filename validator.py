...
def get_name(prompt):
    while True:
        ...
        if name and all(char.isalpha() or char in ["'", " "] for char in name):
            return name
        print("Error: Name is required (letters/spaces only).")

def get_email(prompt):
    while True:
        email = input(prompt).strip().lower()
        if email and "@" in email and "." in email and email.find("@") < email.rfind("."):
            return email
